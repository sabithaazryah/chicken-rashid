            ...
            if ($model->isNewRecord)
                ...
                if (!empty($model->image)) {
                    ...
                    <div class = "col-md-4 img-box">
                        <div class="news-img">
                            <img class="img-responsive" src="<?= Yii::$app->homeUrl ?>../uploads/facility_management_services/services/<?= $model->id ?>/image.<?= $model->image; ?>" />
                            <?= Html::a('<i class="fa fa-remove"></i>', ['/cms/facility-management-details/remove-directory', 'id' => $model->id], ['class' => 'gal-img-remove']) ?>
                        </div>
                    </div>
                    <?php
                }
    ...
    <div class="row">
        <div class='col-md-6 col-sm-6 col-xs-12 left_padd'>
            ...
            <div class="row box-images">
                <?php
                $path = Yii::getAlias('@paths') . '/facility_management_services/partners/' . $model->id;
                    ...
                    foreach (glob("{$path}/*") as $file) {
                        ...
                        if ($img_nmees['1'] != '') {
                            ...
                            <div class = "col-md-4 img-box" id="<?= $k; ?>">
                                <div class="news-img">
                                    <img class="img-responsive" src="<?= Yii::$app->homeUrl . '../uploads/facility_management_services/partners/' . $model->id . '/' . end($arry) ?>">
                                    <?= Html::a('<i class="fa fa-remove"></i>', ['/cms/it-sevices/remove', 'path' => 'facility_management_services/partners/' . $model->id . '/' . end($arry)], ['class' => 'gal-img-remove']) ?>
                                </div>
                            </div>
                            <?php
                        }
        ...
        <div class='col-md-6 col-sm-6 col-xs-12 left_padd'>
            ...
            <div class="row box-images">
                <?php
                $path = Yii::getAlias('@paths') . '/facility_management_services/project_gallery/' . $model->id;
                if (count(glob("{$path}/*")) > 0) {
                    $k = 0;
                    foreach (glob("{$path}/*") as $file) {
                        $k++;
                        ...
                        if ($img_nmees['1'] != '') {
                            ...
                            <div class = "col-md-4 img-box" id="<?= $k; ?>">
                                <div class="news-img">
                                    <img class="img-responsive" src="<?= Yii::$app->homeUrl . '../uploads/facility_management_services/project_gallery/' . $model->id . '/' . end($arry) ?>">
                                    <?= Html::a('<i class="fa fa-remove"></i>', ['/cms/facility-management-details/remove', 'path' => 'facility_management_services/project_gallery/' . $model->id . '/' . end($arry)], ['class' => 'gal-img-remove']) ?>
                                </div>
                            </div>
                            <?php
                        }
...
<script>
    $(document).ready(function () {
        $('#facilitymanagementdetails-service').keyup(function () {
            var name = slug($(this).val());
            $('#facilitymanagementdetails-canonical_name').val(slug($(this).val()));
        });
    });
