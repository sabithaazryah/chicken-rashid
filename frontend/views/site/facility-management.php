        ...
        <div class="row">
            <div class="col-lg-4">
                <div class="service-categories">
                    <h2 class="head active"><a data-toggle="collapse"  href="" role="button">facility management</a></h2>
            ...
            <div class="col-lg-8">
                <div class="service-cont-box">
                    ...
                    if ($facility_service->image != '') {
                        $dirPath = Yii::getAlias(Yii::$app->params['uploadPath']) . '/uploads/it/services/' . $facility_service->id . '/' . $facility_service->id . '.' . $facility_service->image;
                        if (file_exists($dirPath)) {
                            echo '<div class="img-box"><img class="img-fluid" src="' . Yii::$app->homeUrl . 'uploads/it/services/' . $facility_service->id . '/' . $facility_service->id . '.' . $facility_service->image . '"/> </div>';
                        } else {
                            echo '';
                        }
                    } else {
                    ...
                    ?>
                    <?php
                    if ($facility_service->equipment_list != '') {
                        $it_equipment_lists = explode(",", $facility_service->equipment_list);
                    ...
                    <div class="clear"></div>
                    <?php
                    $path1 = Yii::getAlias('@paths') . '/facility_management_services/partners/' . $facility_service->id;
                    if (count(glob("{$path1}/*")) > 0) {
                        ?>
                        <?php if ($facility_service->sub_title != '') { ?>
                            <h3 class="service-head"><?= $facility_service->gallery_title ?></h3>
                        <?php } else { ?>
                            ...
                            <div class="row">
                                ...
                                foreach (glob("{$path1}/*") as $file) {
                                    ...
                                    if ($img_nmees['1'] != '') {
                                        ...
                                        <div class="col-md-4">
                                            <div class="brand-box">
                                                <img src="<?= Yii::$app->homeUrl . 'uploads/facility_management_services/partners/' . $facility_service->id . '/' . end($arry) ?>" alt="it-partner-gallery" class="img-fluid">
                                            </div>
                                        </div>
                                        <?php
                                    }
                    ...
                    ?>
                    <?php
                    $path = Yii::getAlias('@paths') . '/facility_management_services/project_gallery/' . $facility_service->id;
